...
export type NtpStatus = 'pending_review' | 'issued' | 'rejected';

export interface NtpReview {
    id: number;
    ntp_no: string;
    contractor: string;
    status: NtpStatus;
...
export const peso = (n: number) => `PhP ${n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

// Status → display label + badge colors. "pending_review" is shown as "For Review".
export const STATUS_META: Record<NtpStatus, { label: string; bg: string; color: string }> = {
...
// ── Reject modal ─────────────────────────────────────────────────────────────
export function RejectModal({ title, heading, routeName, id, onClose }: {
    title: string;
    heading: string;
    routeName: string;
    id: number;
    ...
    const submit = () => {
        setSaving(true);
        router.patch(route(routeName, id), { remarks }, {
            preserveScroll: true,
            onSuccess: onClose,
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
            <div style={{ background: '#fff', borderRadius: '12px', width: '100%', maxWidth: '460px', overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.25)' }}>
                <div style={{ padding: '16px 22px', borderBottom: '1px solid #f1f5f9', background: '#fef2f2' }}>
                    <h3 style={{ margin: 0, fontSize: '15px', fontWeight: 800, color: '#b91c1c' }}>{heading}</h3>
                </div>
...
// ── NTP card ─────────────────────────────────────────────────────────────────
// An NTP that has been settled either way is closed business, and its quotation
// table is long, so issued and rejected cards start collapsed. Only the records
// still awaiting a decision open on their own; every card folds either way.
export default function NtpReviewCard({ ntp, canAct, approveLabel, onApprove, onReject }: {
    ntp: NtpReview;
    canAct: boolean;
    approveLabel: string;
                ...
                <>
                    ...
                    {ntp.status !== 'pending_review' && (
                        <div style={{ marginTop: '12px', padding: '10px 14px', borderRadius: '8px', background: ntp.status === 'issued' ? '#f0fdf4' : '#fef2f2', border: `1px solid ${ntp.status === 'issued' ? '#bbf7d0' : '#fecaca'}`, fontSize: '12.5px', color: '#475569' }}>
                            {ntp.status === 'issued'
                                ? <span><strong style={{ color: '#166534' }}>Issued</strong>{ntp.issued_date ? ` on ${ntp.issued_date}` : ''} after the full approval chain signed off.</span>
