...
export default function Create({ jobTypes, jobLocations, costCodes }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (requiresCostCode && !form.costcode) {
            setErrors(p => ({ ...p, costcode: 'Cost code is required when OPEX or CAPEX is selected.' }));
            return;
        }
        setErrors({});
        ...
        // Attachments are optional — submit whatever (if any) was added.
        setAttachmentError('');
        setProcessing(true);

        const attachments = [...pictureRows, ...drawingRows, ...reportRows, ...otherRows]
            .filter(row => row.file)
    ...
    return (
        <AuthenticatedLayout>
            ...
            <form onSubmit={handleSubmit} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '28px 30px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <SectionTitle>Supporting Documents &amp; Media (Optional)</SectionTitle>
                {attachmentError && (
